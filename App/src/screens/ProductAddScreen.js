...
class ProductAddScreen extends Component{
  ...
  async reportOnPress(){
    const barcode_id = this.props.route.params?.barcode_id; 
    const barcode_format = this.props.route.params?.barcode_format; 
    ...
    .then((response) => Alert.alert("O produto foi Cadastrado!"))    
    .catch((error) => Alert.alert("Erro ao cadastrar produto"))
    .finally(() => {
      this.props.navigation.goBack();
    });
  }  
    ...
    return (
      <View  style={{flex: 1}}>
        <View style={{flex: 1, alignItems:'center'}}>
          ...
          <Text style={{color: colors.text}}>O produto é livre de componentes animais?</Text>
            <Switch value={this.state.isVegan} color={colors.primaryColor}
            onValueChange={this.toggleSwitch} 
            />
          </View>
          <View style={styles.pickerRow}>
            <Text style={{color: colors.text}}>Fabricante do produto</Text>
            <View style={styles.pickerContainer}>
              <Picker
                selectedValue={this.state.company}
                style={{ height: 50, width: 150, color: colors.textInput}}
                onValueChange={(itemValue, itemIndex) => this.setState({...this.state, company: itemValue})}
              >
              {
